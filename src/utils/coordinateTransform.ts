...
export function calculateViewportTransform(iframe: HTMLIFrameElement | null): ViewportTransform {
  ...
  try {
    ...
    const MANUAL_OFFSET_Y: number = 220; // Add/subtract pixels to fix Y-axis offset (increased to move remote mouse UP)
    
    if (MANUAL_OFFSET_X !== 0 || MANUAL_OFFSET_Y !== 0) {
      console.debug('🔧 [CoordinateTransform] Applying manual offset correction:', { x: MANUAL_OFFSET_X, y: MANUAL_OFFSET_Y });
